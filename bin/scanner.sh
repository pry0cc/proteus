...
cat scope.txt | subfinder -json -o subs.json | jq --unbuffered -r '.host' | dnsx -json -o dnsx.json | jq --unbuffered -r '.host' | httpx -json -o http.json | nuclei -o nuclei.txt -severity info,low,medium,high,critical -t ~/nuclei-templates --stats | notify -config "$ppath/config/notify.yaml" -mf "New vuln found! {{data}}"
...
notify -bulk -i "$raw_path/hosts.txt.new"  -config "$ppath/config/notify.yaml" -mf "New Hostnames Found! {{data}}"

cat http.json | jq -r '.url' | anew "$raw_path/urls.txt" > "$raw_path/urls.txt.new"
notify -bulk -i "$raw_path/urls.txt.new"  -config "$ppath/config/notify.yaml" -mf "New URLs found! {{data}}"
